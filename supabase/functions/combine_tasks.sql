...
CREATE OR REPLACE FUNCTION public.combine_tasks(p_task_ids uuid[])
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  FOR task_record IN
    SELECT id, status FROM tasks WHERE id = ANY(p_task_ids) AND company_id = v_company_id
  LOOP
  ...
  IF array_length(p_task_ids, 1) < 2 THEN
    RAISE EXCEPTION 'At least two tasks required for combining';
  END IF;

  -- Create combined group id
  v_combined_group_id := uuid_generate_v4();

  -- Update tasks
  UPDATE tasks
  SET combined_group_id = v_combined_group_id, status = 'claimed', assigned_user_id = v_user_id
  WHERE id = ANY(p_task_ids);

   -- Audit log
   INSERT INTO audit_logs (company_id, user_id, entity_type, entity_id, action, diff)
   VALUES (v_company_id, v_user_id, 'task', v_combined_group_id, 'combine', jsonb_build_object('task_ids', p_task_ids))
   ...
   VALUES (v_company_id, v_combined_group_id, v_undo_token, now() + interval '1 hour');

   -- Realtime notify for each task
  FOR task_record IN SELECT id FROM tasks WHERE id = ANY(p_task_ids) LOOP
    PERFORM pg_notify('realtime', json_build_object(
      'type', 'task_update',
      'entity_id', task_record.id,
      'data', jsonb_build_object('combined_group_id', v_combined_group_id, 'status', 'claimed'),
      'actor', v_user_id,
      'timestamp', now()
    )::text);
  END LOOP;

   RETURN json_build_object('success', true, 'combined_group_id', v_combined_group_id, 'undo_token', v_undo_token, 'timestamp', now());
END;
$$;